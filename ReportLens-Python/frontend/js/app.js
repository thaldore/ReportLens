/**
 * ReportLens Frontend Application — Python Environment
 * Tüm API etkileşimleri, SPA routing ve modül mantığı.
 */

const API = '';  // Aynı sunucu — proxy gerekmiyor
...
async function loadDashboard() {
    try {
        ...
        document.getElementById('dash-db-engine').textContent = 'Microsoft Agent Framework';
        document.getElementById('dash-model').textContent = statusData.model || 'llama3.1:8b';

        // Birim filtresini analiz sayfasına da doldur
        ...
        showToast('Dashboard yüklenirken hata: ' + e.message, 'error');
    }
}
...
function renderDashReports(reports) {
    ...
    if (!reports.length) {
        container.innerHTML = '<div class="empty-state"><span class="material-symbols-outlined">folder_off</span><h3>Rapor bulunamadı</h3><p style="font-size:13px">Rapor Yönetimi sayfasından dosya yükleyin.</p></div>';
        return;
    }
...
function renderSystemInfo(status) {
    document.getElementById('dash-system-info').innerHTML = `
        ...
        <div class="insight-alert opportunity">
            <div class="alert-title">Ollama</div>
            <p style="font-size:12px">${status.ollama_url || '—'}</p>
        </div>
    `;
}
...
function renderQuickStats(status, reports) {
    ...
    document.getElementById('dash-quick-stats').innerHTML = `
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;font-size:12px">
            <span style="color:var(--on-surface-variant)">Vektör Noktası</span>
...
async function sendAnalysis() {
    ...
    chatBox.innerHTML += `
        <div class="chat-message" id="${loadingId}">
            ...
            <div class="chat-bubble ai"><div class="loading-container" style="padding:12px"><div class="loading-spinner"></div><div class="loading-text">Analiz ediliyor...</div></div></div>
        </div>
    `;
    ...
    try {
        ...
        const loadingEl = document.getElementById(loadingId);
        if (loadingEl) {
            loadingEl.querySelector('.chat-bubble').innerHTML = `<p style="color:var(--error)">❌ Hata: ${escapeHtml(e.message)}</p>`;
        }
    }
...
async function loadRubricFiles() {
    try {
        ...
        const ozReports = (data.reports || []).filter(r =>
            r.filename.toLowerCase().includes('oz_degerlendirme') ||
            r.filename.toLowerCase().includes('ozdegerlendirme')
        );
...
async function loadConsistencyOptions() {
    try {
        const [birimData, reportData] = await Promise.all([
            apiGet('/api/birimler'),
            apiGet('/api/reports'),
        ]);
...
async function generateAndCheckMock() {
    ...
    try {
        ...
        if (mockText.includes('BOLUM 2')) {
            const parts = mockText.split('BOLUM 2');
            surveyText = parts[0].trim();
            comparisonText = parts[1] ? parts[1].trim() : mockText;
        }

        // 3. Tutarsızlık analizi
        const consData = await apiPost('/api/consistency', {
            comparison_text: comparisonText,
            survey_text: surveyText || null,
            filename,
        });
...
async function loadManagementData() {
    try {
        ...
        if (!files.length) {
            container.innerHTML = '<div class="empty-state"><span class="material-symbols-outlined">folder_off</span><h3>Dosya bulunamadı</h3><p style="font-size:13px">Yukarıdan dosya yükleyin.</p></div>';
            return;
        }
        container.innerHTML = `
...
async function uploadFiles(fileList) {
    ...
    try {
        ...
        showToast(`${data.count} dosya yüklendi`, 'success');
        loadManagementData();
    } catch (e) {
...
async function processAndIndex(forceReindex) {
    ...
    statusEl.innerHTML = '<div style="display:flex;align-items:center;gap:8px"><div class="loading-spinner" style="width:20px;height:20px"></div> İşleniyor...</div>';

    try {
        const data = await apiPost('/api/process?force_reindex=' + forceReindex, {});
        statusEl.innerHTML = `<p style="color:#2e7d32">✅ İşlem tamamlandı</p>`;
        showToast('İşleme ve indeksleme tamamlandı', 'success');
        loadManagementData();
    } catch (e) {
...
async function loadTestResults() {
    try {
        ...
        container.innerHTML = `<div class="report-list">${results.map(r => {
            const m = r.meta || {};
            return `
                <div class="report-item" onclick="viewTestResult('${r.filename}')" style="cursor:pointer">
                    <div class="report-item-left">
                        <div class="report-icon"><span class="material-symbols-outlined">science</span></div>
                        <div>
                            <div class="report-name">${r.filename}</div>
                            <div class="report-meta">${m.tarih || '—'}</div>
                        </div>
                    </div>
                    <div style="text-align:right">
                        <div style="font-size:13px;font-weight:700">${m.toplam_test || '—'} test</div>
                        <div style="font-size:11px;color:#2e7d32;font-weight:600">${m.basarili || '—'} başarılı</div>
                    </div>
                </div>
            `;
        }).join('')}</div>`;
...
async function loadSettings() {
    try {
        ...
        document.getElementById('settings-llm').innerHTML = `
            ...
            <div><strong>Keep Alive:</strong> ${data.keep_alive || '—'}</div>
            <div><strong>Bağlam Penceresi:</strong> ${data.num_ctx || '—'}</div>
        `;
        document.getElementById('settings-db').innerHTML = `
            <div><strong>Sunucu:</strong> ${data.mssql_host || '—'}</div>
            <div><strong>Veritabanı:</strong> ${data.mssql_db || '—'}</div>
        ...
        document.getElementById('settings-sys').innerHTML = `
            <div><strong>Re-ranker:</strong> ${data.reranker_enabled ? '✅ Aktif' : '❌ Devre Dışı'}</div>
...
function exportCurrentResult() {
    ...
    a.href = url;
    a.download = `reportlens_export_${new Date().toISOString().slice(0,10)}.md`;
    a.click();
